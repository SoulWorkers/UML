...
public class textServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (method.equals("getAll")) {
			...
			request.getRequestDispatcher("/admin/backcourseDetail-3.jsp").forward(request, response);
			...
			List<textbook>textbooks=service.getAll();
			request.getSession().setAttribute("books", textbooks);
			request.getRequestDispatcher("/html/courseDetail-3.jsp").forward(request, response);
		}
	}
